    ...
    <section className="relative overflow-hidden bg-[#F5F7FA]">
      ...
      <div className="relative block px-4 py-12 md:hidden">
        ...
        <motion.h2
          ...
          viewport={{ once: true }}
          className="text-center text-4xl font-black leading-tight tracking-tight text-gray-900"
        >
          Premium Tech.
        ...
        {/* Metrics — centered row, value above label */}
        <motion.div
          initial={{ opacity: 0, y: 14 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.55, delay: 0.1 }}
          viewport={{ once: true }}
          className="mt-7 flex items-start justify-center gap-8"
        >
          {metrics.map((m, i) => (
        ...
        {/* Panels — square, edge-to-edge with tiny gap */}
        <div className="mt-7 flex flex-col gap-2">
          {panels.map((panel, i) => (
            ...
            >
              ...
              {/* Corner badge */}
              <div className="absolute right-4 top-4 z-10 rounded-full border border-white/20 bg-white/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-white/70 backdrop-blur-sm">
                {i === 0 ? "New In" : "Featured"}
              </div>

              {/* Content */}
              <div className="absolute bottom-0 left-0 z-10 p-5">
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#7DBBFF]">
                  {panel.eyebrow}
                </p>
                ...
                <Link href={panel.href}>
                  <motion.span
                    whileTap={{ scale: 0.96 }}
                    className="mt-4 inline-flex cursor-pointer items-center gap-2 rounded-md border border-white/25 bg-white/15 px-4 py-2.5 text-[12px] font-bold text-white backdrop-blur-sm"
                  >
                    {panel.cta}
                    <ArrowRight size={12} />
                  </motion.span>
          ...
          {panels.map((panel, i) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 z-10 p-7">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#7DBBFF]">
                  {panel.eyebrow}
                </p>
                <h3 className="mt-3 whitespace-pre-line text-2xl font-black leading-tight text-white md:text-[28px]">
                  {panel.title}
                </h3>
